...
// Canny法用
// 閾値
int CANNY_THRESHOLD_UNDER = 100;
int CANNY_THRESHOLD_UPPER = 200;
...
void myTransform_Hough( PImage image )
{
  ...
  // 距離(垂線の長さ)の最大値
  int RHO_MAX = int( sqrt( pow(image.width,2)+pow(image.height,2) ) );
  // 角度θの配列を作成
  float[] TABLE_THETA = new float[ THETA_SPLIT_MAX ];
  for( int i = 0 ; i < THETA_SPLIT_MAX ; i++ )
  {
    TABLE_THETA[i] = 2*PI/THETA_SPLIT_MAX*i;
  }
  // 三角関数の配列を作成(時間短縮のため)
  ...
  for( int i = 0 ; i < THETA_SPLIT_MAX ; i++ )
  {
    TABLE_SIN[i] = sin( TABLE_THETA[i] );
    TABLE_COS[i] = cos( TABLE_THETA[i] );
  }
  ...
  vote = new int[ RHO_MAX*2 ][];
  // θの次元を追加
  // 垂線の長さの最大値だけループ
  for( int i = 0 ; i < RHO_MAX*2 ; i++ )
  {
    // 次元を確定させる
    vote[i] = new int[ THETA_SPLIT_MAX ];
  }
  // 0で埋める
  for( int i = 0 ; i < RHO_MAX*2 ; i++ )
  {
  ...
  //  for( int theta = 0 ; theta < THETA_SPLIT_MAX ; theta++ )
  //  {
  //    if( vote[ rho ][ theta ] < 10 )
  //    {
  //      continue;
  //    }
  //    println( "("+theta+","+rho+")="+vote[ rho ][ theta ] );
  //  }
  //}
  ...
  while( count < LINE_MAX )
  {
    ...
    // 表示
    println( count+":(theta,rho)="+"("+theta_max+","+rho_max+")="+vote[ rho_max ][ theta_max ] );
    ...
    for( int x = 0 ; x < image.width ; x++ )
    {
      // y座標を計算する
      int y;
      y = int( rho_max/TABLE_COS[ theta_max ] - x*TABLE_COS[ theta_max ]/TABLE_SIN[ theta_max ] );
      // 求めたy座標が画像の外に出ていた場合
    ...
    for( int y = 0 ; y < image.height ; y++ )
    {
      // x座標を計算する
      int x;
      x = int( rho_max/TABLE_SIN[ theta_max ] - y*TABLE_SIN[ theta_max ]/TABLE_COS[ theta_max ] );
      // 求めたy座標が画像の外に出ていた場合
    ...
    for( int rho = -20 ; rho < 20 ; rho++ )
    {
      for( int theta = -20 ; theta < 20 ; theta++ )
      {
        ...
        // 位置が投票結果用配列の外側に来た場合
        if(  pos_rho < 0 || pos_theta < 0 || pos_rho >= RHO_MAX*2 || pos_theta >= THETA_SPLIT_MAX )
        {
          continue;  // 処理しない
        }
        // 位置が投票結果用配列の内側に来た場合
